...
//! @version 1.0
//!
//! @author Kosenkov Ivan, Morochev Georgiy
//! @date   03.04.2017
//-----------------------------------------------------------------------------
#ifndef BIG_EXCEPTIONS_H
#define BIG_EXCEPTIONS_H 1
...
#define BIG_OUT_OF_RANGE(condition) {\
    if ((condition)) {\
        throw big::outOfRange(FULL_COORDINATES_FFL);\
    }\
}

#define BIG_BAD_ALLOC(condition) {\
    if ((condition)) {\
        throw big::badAlloc(FULL_COORDINATES_FFL);\
    }\
}

#define BIG_INVALID_ARGUMENT(condition) {\
    if ((condition)) {\
        throw big::invalidArgument(FULL_COORDINATES_FFL);\
    }\
}

#define BIG_INVALID_OBJECT(condition) {\
    if ((condition)) {\
        throw big::invalidObject(FULL_COORDINATES_FFL);\
    }\
}

#define BIG_BAD_STREAM(condition) {\
    if ((condition)) {\
        throw big::badStream(FULL_COORDINATES_FFL);\
    }\
}

#define BIG_OTHER_ERROR(condition, mesg) {\
    if ((condition)) {\
        throw big::otherError(FULL_COORDINATES_FFL, (mesg));\
    }\
}

#endif // BIG_EXCEPTIONS_H